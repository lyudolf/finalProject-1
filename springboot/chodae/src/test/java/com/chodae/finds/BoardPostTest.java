...
//	String kind = "경기","초보","자바"   => 프론트에서 이렇게 3개의 카테고리가 넘어오면 게시글번호를 외래키로 카테고리테이블에서 3개를 저장하는 방식으로 가야될 것 같다. 
	//그러므로 게시글 테이블에서 카테고리 필드는 빼버리고 대신 카테고리테이블에서 게시글 번호로 다대일 양방향 관계설정을 해주면 될 것 같다. 일단은. 해보고 테스트 해볼것.
	//게시글 조회시 카테고리는 join으로 조회하게 될듯 
	//
