...
function ResetPs() {
    ...
    const onChangePs = event => {
        ...
        //형식 체크 
        const pattern = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&])[A-Za-z\d$@$!%*#?&]{8,20}$/; // 비밀번호는 8~20자 영문숫자 특수문자 혼합, 대소문자 구별
    ...
    const submitForm = async function (event) {
        ...
        if (!(ps === ps2)) {
            ...
            await axios.put("/api/login/find/ps", formData, {
                headers: {
    ...
    return (
        <div className='mainBody3'>
            <div className='navbar3'>
                <div className='navbarHead3'>아이디/비밀번호 찾기</div>
                <div className='navbarBody3'>
                    <ul>
            ...
            <div className='container3'>
                <div className='title3'>비밀번호 찾기</div>
                <hr></hr>
                <li>
                ...
                <div className='content3'>
                    <div className='contentHeader3'>비밀번호를 재설정하기</div>
                    <div className='contentHeader3'>안전한 사용을 위해 기존 비밀번호를 바꿔주세요.</div>
                    <hr />
                    <form className="searchForm3">
                        <div className="nameInput3">
                            ...
                            />
                            {(!isValidPs || !isSamePs) && <div className='errorMessage'>{psMsg}</div>}

                        </div>
                        <div className="emailInput3">
                            ...
                            />
                            {(!isValidPs2 || !isSamePs) && <div className='errorMessage2'>{psMsg2}</div>}

                        </div>
                        <div><button type='submit' className='searchBtn3' onClick={submitForm}>재설정하기</button></div>
                        <div>
                            <Link to="/">
                                <button type='button' className='backBtn3'> 로그인 화면으로 돌아가기 </button>
                            </Link>
                        </div>
                    </form>
                </div>
