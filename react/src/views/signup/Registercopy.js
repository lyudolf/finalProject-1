...
const Registercopy = () => {
  ...
  const validate = Yup.object({
    ...
    nickname: Yup.string()
      ...
      .test("중복검사", "이미 사용중인 닉네임 입니다",
        function (values) {
          ...
          const promise1 = axios.get('http://localhost:8001/study/me/nickname', { params: { nickname: nickname } })
            .then((res) => {
    ...
    email: Yup.string()
      ...
      .test("중복검사", "이미 사용중인 이메일입니다",
        function (values) {
          const email = values;
          const promise = axios.get('http://localhost:8001/study/me/email', { params: { email: email } })
            .then((res) => {
              console.log(res.data)
              if (!res.data) {
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        const { name, password, nickname, email } = values;
        console.log(values);
        axios.post('http://localhost:8001/study', { email, name, password, nickname }, {
          "headers": {
                  ...
                  <div>
                    ...
                    <span className="box int_email">
                      <input
                        className="int"
                        placeholder="아이디"
                        type="text" {...formik.getFieldProps('id')}
                      />
                    </span>
