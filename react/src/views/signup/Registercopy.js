...
const Registercopy = () => {
  ...
  const validate = Yup.object({
    ...
    nickname: Yup.string()
      ...
      .test("중복검사", "이미 사용중인 닉네임 입니다",
        function (values) {
          const nickname = values;
          const promise1 = axios.get('http://localhost:8000/reg/me/nickname', { params: { nickname: nickname } })
            .then((res) => {
    ...
    email: Yup.string()
      ...
      .test("중복검사", "이미 사용중인 이메일입니다",
        function (values) {
          const email = values;
          const promise = axios.get('http://localhost:8000/reg/me/email', { params: { email: email } })
            .then((res) => {
              console.log(res.data)
              if (!res.data) {
    ...
    loginId: Yup.string()
      .required('필수 정보입니다')
      .test("중복검사", "이미 사용중인 아이디 입니다",
        function (values) {
          const loginId = values;
          const promise = axios.get('http://localhost:8000/reg/me/loginId', { params: { loginId: loginId } })
            .then((res) => {
              console.log(res.data)
              if (!res.data) {
                console.log("같은 아이디 없어");
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        const { name, password, nickname, email, loginId, campus, language, phone, mailAds, } = values;
        console.log(values);
        console.log(values.password)
        axios.post('http://localhost:8000/reg', { name, password, nickname, email, loginId, campus, language, phone, mailAds, }, {
          "headers": {
              ...
              <div id="wrapper">
                <div id="content">
                  ...
                  <div>
                    ...
                    <span className="box int_email">
                      <input
                        className="int"
                        placeholder="아이디"
                        type="text" {...formik.getFieldProps('loginId')}
                      />
                    </span>
                  ...
                  <div>
                    ...
                    <span className="box int_pass">
                      <input
                        ...
                        type="password"
                        className="int"
                        {...formik.getFieldProps('password')} placeholder="8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요."
                  ...
                  <div>
                    ...
                    <span className="box int_pass_check">
                      <input
                        type="password"
                        className="int"
                        {...formik.getFieldProps('confirmPassword')} placeholder="비밀번호확인"
                  ...
                  <div className="checkb">
                    <input type="checkbox" {...formik.getFieldProps('mailAds')} />
