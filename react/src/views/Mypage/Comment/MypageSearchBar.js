...
import styles from "./MypageSearchBar.module.css";
import axios from "../../../plugins/axios";

function SearchBar({ getData, nickname }) {
  ...
  const [searchBy, setSearchBy] = useState("content"); //초기값은 드롭박스 처음 값으로 설정.
  ...
  const searchList = async function (searchBy, searchTerm) {
    ...
    await axios
      ...
      .then((response) => {
        ...
        navigate(
          `/mypage/MypageComment?page=${1}&searchType=${searchBy}&keyword=${searchTerm}`
        );
      })
      .catch((error) => {
        console.log(error);
