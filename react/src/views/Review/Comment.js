...
const Comments = ({ match, history }) => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:3000/allreply", null, {
        params: {
          BOARDNO: urlparams[1]
        }
      })
      .then((response) => {
        ...
        for (let i = 0; i < response.data.length; i++) {
          ...
          if (urlparams[0] == response.data[i].idmanage.id) {
            ...
            html += <button class="delete_reply ${response.data[i].cnumber}">삭제하기</button>;
          }
          html += `</div>`
        }
        ...
        $('#replybutton').on('click', () => {
          ...
          if (context != "") {
            ...
            axios
              .post("http://localhost:3000/addreply", null, {
                params: {
        ...
        $(".check_reply").on('click', (e) => {
          ...
          axios
            .post("http://localhost:3000/updatereply", null, {
              params: {
        ...
        $(".delete_reply").on('click', (e) => {
          console.log(e.target.classList[1]);
          axios
            .post("http://localhost:3000/deletereply", null, {
              params: {
